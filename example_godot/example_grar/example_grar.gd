...
const EXTRACT_DIR = "user://grrar_extracted"

func _ready():
	$Label.text = "Listo. Selecciona un archivo .rar para probar Grrar."
...
func _on_test_pressed() -> void:
	...
	if path == "":
		$Label.text += "Error: Introduce una ruta de archivo .rar\n"
		return
	...
	# 1. Listar entradas
	$Label.text += "\n1. Leyendo entradas con Grrar:\n"
	var entries
	if pass == "":
		entries = grrar.get_entries(global)
	else:
		entries = grrar.get_entries_with_password(global, pass)
	...
	for entry in entries:
		...
		if entry["is_encrypted"]: info += " [ENC]"
		if entry["is_split"]: info += " [SPLIT]"
		$Label.text += info + "\n"
	...
	if not entries[0]["is_directory"]:
		var bytes
		if pass == "":
			bytes = grrar.extract_entry_to_buffer(global, first_name)
		else:
			bytes = grrar.extract_entry_to_buffer(global, first_name, pass)
		if bytes.size() > 0:
	...
	var success
	if pass == "":
		success = grrar.extract_all(global, absolute_extract_dir)
	else:
		success = grrar.extract_all(global, absolute_extract_dir, pass)
	if success:
	...
	if not entries[0]["is_directory"]:
		...
		var ok
		if pass == "":
			ok = grrar.extract_entry(global, first_name, single_dest)
		else:
			ok = grrar.extract_entry(global, first_name, single_dest, pass)
		if ok:
