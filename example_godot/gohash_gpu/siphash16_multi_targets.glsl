...
layout(set = 0, binding = 0, std430) restrict readonly buffer Targets {
...
layout(set = 0, binding = 2, std430) restrict buffer Found {
	uint target_found[];
};

layout(set = 0, binding = 3, std430) restrict readonly buffer TargetValues {
	uint target_values[];
};
...
void main() {
	uint value_id = gl_GlobalInvocationID.x;
	uint target_id = gl_GlobalInvocationID.y;

	if (value_id >= params.total_values || target_id >= params.target_count) {
		return;
	}

	uint value = params.start_value + value_id;
	u64x hash64 = siphash13_one_u32(value);
	uint hash16 = hash64.low & 0xffffu;

	if (hash16 == target_hash16_values[target_id]) {
		atomicAdd(match_counts[target_id], 1u);

		if (value == target_values[target_id]) {
			target_found[target_id] = 1u;
		}
	}
}
