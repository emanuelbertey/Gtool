...
layout(set = 0, binding = 0, std430) restrict buffer Results {
	uint hash16_values[];
};
...
void main() {
	...
	if (id >= params.total_values) {
		return;
	}

	uint value = params.start_value + id;
	u64x hash64 = siphash13_one_u32(value);
	uint hash16 = hash64.low & 0xffffu;

	hash16_values[id] = hash16 == params.target_hash16 ? value : 0xffffffffu;
}
